...
def get_balance(bank_account):
    last_transaction = (
        BankTransaction.objects.filter(bank_account__id=bank_account["id"])
...
def get_month_and_year_transaction(bank_account):
    ...
    month_transactions = BankTransaction.objects.filter(
        bank_account__id=bank_account["id"]
    ).filter(date__month=current_month)
    year_transactions = BankTransaction.objects.filter(
        bank_account__id=bank_account["id"]
...
def transaction_exists(transaction):
    existing_transaction = (
        BankTransaction.objects.filter(bank_account=transaction["bank_account"])
...
def get_transactions_by_period(bank_account, period, date_range):
    if period in ["month", "year"]:
        month_transactions, year_transactions = get_month_and_year_transaction(
            bank_account
        )
    ...
    if period == "custom":
        ...
        transactions = get_transactions_by_date_range(
            bank_account, start_date, end_date
        )
        return transactions
    return []
