...
urlpatterns = [
    ...
    re_path(
        r"^accounts/(?P<pk>\d+)/transactions/period/$",
        TransactionsPeriodViewSet.as_view({"post": "list"}),
    ),
    re_path(
        r"^accounts/(?P<pk>\d+)/overview/period/$",
        OverviewPeriodViewSet.as_view({"post": "list"}),
    ),
]
